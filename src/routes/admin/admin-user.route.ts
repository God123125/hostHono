import { Hono } from "hono";
import { adminUserController } from "../../services/admin/admin-user.service.js";
const routes = new Hono();
routes.get("/", adminUserController.getUsers);
routes.get("/search", adminUserController.search);
routes.get("/get-overall-data", adminUserController.getMerchantOverallStats);
routes.get("/commissions", adminUserController.getCommissions);
routes.post("/", adminUserController.create);
routes.post("/login", adminUserController.login);
routes.get("/profile/:id", adminUserController.getUserProfile);
routes.patch("update-info/:id", adminUserController.updateAccountInfo);
routes.patch("update-profile/:id", adminUserController.updateProfile);
routes.patch("/update-commissions/:id", adminUserController.updateComission);
routes.delete("/:id", adminUserController.delete);
routes.get("/:id", adminUserController.getById);
export default routes;
